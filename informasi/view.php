<div class="table-responsive">
	<table class="table table-striped table-bordered table-hover">
		...
		if(isset($_POST['search']) && $_POST['search'] == true){ // Jika ada data search yg dikirim (user telah mengklik tombol search) dan search sama dengan true
			...
			$sql = mysqli_query($connect, "SELECT * FROM sekilasinfo WHERE info LIKE '".$param."' OR tgl_posting LIKE '".$param."' LIMIT ".$limit_start.",".$limit);

			// Buat query untuk menghitung semua jumlah data
			// dengan keyword yang telah di input
			$sql2 = mysqli_query($connect, "SELECT COUNT(*) AS jumlah FROM sekilasinfo WHERE info LIKE '".$param."' OR tgl_posting LIKE '".$param."'");
			$get_jumlah = mysqli_fetch_array($sql2);
		}else{ // Jika user belum mengklik tombol search (PROSES TANPA AJAX)
			// Buat query untuk menampilkan semua data siswa
			$sql = mysqli_query($connect, "SELECT * FROM sekilasinfo LIMIT ".$limit_start.",".$limit);

			// Buat query untuk menghitung semua jumlah data
			$sql2 = mysqli_query($connect, "SELECT COUNT(*) AS jumlah FROM sekilasinfo");
			$get_jumlah = mysqli_fetch_array($sql2);
		}
		...
		while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
			...
			<tr>
				...
				<td class="align-middle"><?php echo $data['info']; ?></td>
				<td class="align-middle"><?php echo $data['tgl_posting']; ?></td>
			</tr>
			<?php
			$no++;
		}
